...
function gen_Ap(dfC::DFController,p::Int)
	...
	if k<N-1
		...
		Ap[1:n,1:m+s+n]=[B1 zeros(n,s) -In];
		for i=1:N-k-2
...
function get_δ(dfC::DFController,x)
	...
	return bigJ*x[N*(n+m)+1:length(x)];
end
...
function set_Problem(dfC::DFController)
	...
	@variable(dfC.model, PExInp[1:nExInp,1:N] in Parameter(0.0));
	#Generate matrices
	...
	DD=dfC.DD;
	#Set constraints
	@variable(dfC.model,pre_x[1:Int.(size(AA,2)-l*m*(mstruct*(2*N-1-mstruct)/2))]);
	dfC.x=construct_x(dfC,pre_x);
	@constraint(dfC.model, c1, AA*dfC.x.==BB);
	if dfC.isSoft && length(dfC.softInds)>0
		liftedSoftInds=kron(ones(1,N),(dfC.softInds)')+kron((0*s:s:(N-1)*s)',ones(1,length(dfC.softInds)));
		...
		Dsoft=DD[1:N*s+r,1]+softLifted;
		Dsoft=[Dsoft; convert(Vector{AffExpr},DD[N*s+r+1:length(DD)])];
		@constraint(dfC.model, c2, CC*dfC.x.<=Dsoft);
		@constraint(dfC.model,cSoft, dfC.soft.>=0)
	else
		@constraint(dfC.model, c2, CC*dfC.x.<=DD)
	end
end
...
function calc_Inp(dfC::DFController,x0,ExInp,costParams,disturbance)
	...
	if is_solved_and_feasible(dfC.model)
		...
		J=kron(Matrix{Float64}(I,N,N),E);
		dfC.lastM=U*(J\ Matrix{Float64}(I,N*n,N*n));
		dfC.lastv= JuMP.value.(v);
		dfC.lastDistSequence=zeros(N*n,1);
		...
		dfC.lastDistSequence[(distInd-1)*n+1:distInd*n]=disturbance;
		...
		return currentInP, [A*x0+B1*currentInP+B2*ExInp[:,1]]
	end
end
